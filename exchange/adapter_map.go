...
import (
	...
	"github.com/prebid/prebid-server/adapters/emxdigital"
...
func newAdapterMap(client *http.Client, cfg *config.Configuration, infos adapters.BidderInfos) map[openrtb_ext.BidderName]adaptedBidder {
	ortbBidders := map[openrtb_ext.BidderName]adapters.Bidder{
		...
		openrtb_ext.BidderEmx:        emxdigital.NewEmxBidder(cfg.Adapters[string(openrtb_ext.BidderEmx)].Endpoint),
	...
	legacyBidders := map[openrtb_ext.BidderName]adapters.Adapter{
		...
		// NICK THIS NEEDS TO BE UPDATED -- JUST DEV
		// openrtb_ext.BidderEmx: emxdigital.NewEmxAdapter(adapters.DefaultHTTPAdapterConfig, cfg.Adapters[string(openrtb_ext.BidderEmx)].Endpoint),
	}
